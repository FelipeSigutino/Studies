...
public static TPlayer createPlayer1(int CONTROL) {
...
public static TPlayer createPlayer2(int CONTROL) {
    TPlayer p = new TPlayer();
    p.Points = 0;
    p.X = 115;
    p.Y = 15;
...
public static TSquareGroup createSquares(int count) {
    ...
    g.bounced = new boolean[count];
    g.initialY = new int[count];
...
public static void movePlayer2(TPlayer p,TSquareGroup g) {
    ...
    DeleteHitSquare(p,g,800);
}

public static void DeleteHitSquare(TPlayer p, TSquareGroup g , int SOUND) {
    for (int i = 0; i < g.n; i++) {
...
public static void DeleteHitSquare1(TPlayer p, TSquareGroup g , int SOUND) {
    for (int i = 0; i < g.n; i++) {
    ...
    ChosenSquareDead = true;
}

public static int Distance(TPlayer p, TSquareGroup g, int i) {
    int sx = g.X[i];
    int sy = g.Y[i];

    int dx = Math.abs(p.X - sx);
    int dy = Math.abs(p.Y - sy);
    return dx + dy;
}


public static void moveAI1(TPlayer p,TSquareGroup g) {
    int LowestDistance = Integer.MAX_VALUE;
    int BestSquare = 0;
    for (int i = 0; i < g.n; i++) {
        if (Distance(p, g, i) <  LowestDistance) {
            LowestDistance = Distance(p, g, i);
            BestSquare = i;
        }
    }
    if (g.X[BestSquare] > p.X) {
        p.X++;
    } else if (g.X[BestSquare] < p.X) {
        p.X--;
    } else if (g.Y[BestSquare] > p.Y) {
        p.Y++;
    } else if (g.Y[BestSquare] < p.Y) {
        p.Y--;
    }
...
public static int BestSquare = 0;

public static Boolean ChosenSquareDead = true;

public static void moveAI2(TPlayer p,TSquareGroup g) {
    if(ChosenSquareDead) {
        int BestSquare = rand.nextInt(g.n);
        ChosenSquareDead = false;
    }
    if (g.X[BestSquare] > p.X) {
        p.X++;
    } else if (g.X[BestSquare] < p.X) {
...
public static int[] loadControlsFromConfig() {
    int[] controls = {0, 0}; // defaults

    try (BufferedReader br = new BufferedReader(new java.io.FileReader("config.txt"))) {
        String line;
        while ((line = br.readLine()) != null) {
            line = line.trim().toLowerCase();
            if (line.startsWith("playerone")) {
                if (line.contains("1")){
                    controls[0] = 1;
                }
            } else if (line.startsWith("playertwo")) {
                if (line.contains("1")){
                    controls[1] = 1;
                }
            }
        ...
        System.out.println("Could not read config.txt – using defaults.");
    }

    return controls;
}

public static int[] controls = loadControlsFromConfig();
...
public static void main() {
    ...
    TSquareGroup squares = createSquares(10);
    TPlayer Player1 = createPlayer1(0);
    TPlayer Player2 = createPlayer2(0);
    ...
    while (!quit) {
        ...
        switch (Player1.CONTROL) {
            ...
            case 1 -> {
                moveAI1(Player1, squares);
                break;
            }
            case 2 -> {
                moveAI2(Player2, squares);
            }
        }
