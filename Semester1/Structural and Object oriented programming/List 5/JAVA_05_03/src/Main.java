...
public static void DrawTimeToBuffer() {
    ...
    buffer[0][0] = WHITE + "Timeleft: " + Timeleft;
    if (0 == Timeleft) {
        quit = true;
    }
}

public static void DrawScoreToBuffer(TPlayer p) {
    buffer[15][0] = WHITE + "Score: " + p.Points;
}
...
public static void movePlayer(TPlayer p,TSquareGroup g) {
    if (keypressed()) {
...
public static void main() {
    ...
    TPlayer Player = createPlayer();

    while (!quit) {
        clearBuffer();
        move(squares);
        for (int i = 0; i < 5; i++) {
            movePlayer(Player, squares);
            delay(7);
        }

        DrawTimeToBuffer();
        DrawFrameToBuffer();
        DrawPlayertoBuffer(Player);
        DrawScoreToBuffer(Player);
        render(121,31);
